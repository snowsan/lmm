window.onload = function () {
    document.getElementById("img").onclick = function () {
        ...
        document.getElementById("file").onchange = function () {
            ...
            //发送表单放到FormData中，不用setRequestHeader
            var fd = new FormData(f);
            var xh = new XMLHttpRequest();
            xh.open("post", "/ServletUpload");
            xh.send(fd);
            xh.onreadystatechange = function () {
                if (xh.readyState == 4) {
                    if (xh.status == 200 || xh.status == 304) {
                        document.getElementById("img").src = xh.responseText;
                    }
                }
            }
        }
...
//处理修改按钮
function modify() {
    var ooo = document.getElementById("save");
    ooo.removeAttribute("hidden");
    var oo = document.getElementById("sexs");
    ...
    for (var i = 0; i < o.length; i++) {
        ...
        o[i].className += " add-border";
    }
}

function save() {
    var ooo = document.getElementById("sexs");
    ooo.setAttribute("disabled", "disabled");
    var oo = document.getElementById("save");
    oo.setAttribute("hidden", "hidden");
    var o = document.getElementsByClassName("fildtext");
    for (var i = 0; i < o.length; i++) {
        o[i].setAttribute("disabled", "disabled");
        o[i].setAttribute("readonly", "readonly");
    }
    ...
    var fd = new FormData(f);
    var xh = new XMLHttpRequest();
    xh.open("post", "/ServletUpload1");
    xh.send(fd);
    xh.onreadystatechange = function () {
        if (xh.readyState == 4) {
            if (xh.status == 200 || xh.status == 304) {
                document.getElementById("aaa").innerHTML = xh.responseText;
            }
